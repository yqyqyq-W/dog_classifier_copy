...
def main():
    ...
    tr_loader, va_loader, te_loader, semantic_labels = get_train_val_test_loaders(
        task="source", batch_size=config("challenge_source.batch_size")
    )

    model = Challenge_Source()
    print("Loading source...")
    model, epoch, stats = restore_checkpoint(model, config("challenge_source.checkpoint"))

    sem_labels = "0 - Samoyed\n1 - Miniature Poodle\n2 - Saint Bernard\n3 - Great Dane\n4 - Dalmatian\n5 - Chihuahua\n6 - Siberian Husky\n7 - Yorkshire Terrier"

    # Evaluate model
    plot_conf(va_loader, model.eval(), sem_labels, "challenge_conf_matrix.png")
